        ...
        <div className="modal-box w-2/3 max-w-1xl py-5 px-12 overflow-visible ">
            ...
            <div className={`${centering} relative justify-between mt-10`}>
                <div className="flex text-D3D3D3 text-2xl">
                    <span className={`${priority.style} cursor-pointer border-none`} onClick={()=> setPriority({...priority,modal : true})} ><FiFlag /></span>
                    <span className='mr-5 text-208D8E text-xl font-medium'>{todayDate}</span>

                    {priority.modal && 
                        <PriorityOptions handlePriority={handlePriority} className='top-7'/>
                    }
                </div>
